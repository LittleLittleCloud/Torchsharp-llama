...
public static class Utils
{
    public static Tensor ApplyRotaryEmbeddings(Tensor input, Tensor freqsComplex)
    {
        ...
        var input_complex = input.reshape(input.shape[0], input.shape[1], input.shape[2], -1, 2).view_as_complex();
...
//    return freqs_complex
    ...
    public static Tensor PrecomputeThetaPosFrequencies(int headDim, int seqLen, string device, float theta = 10000.0f)
    {
        ...
        // Shape: (Head_Dim / 2)
        var thetaInput = 1.0f / (torch.pow(theta, -2.0f * (thetaNumerator / headDim))).to(device); // (Dim / 2)
